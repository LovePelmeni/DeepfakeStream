...
class InferenceModel(object):
    ...
    def from_config(cls, config_path: str):
        ...
        try:
            # image size, acceptable by the encoder
            encoder_image_size = inf_config.get("encoder_image_size")
            # minimum size of the face to detect
            min_face_size = inf_config.get("min_face_size", 160)
            # margin to apply to face crop after
            face_margin_size = inf_config.get("face_margin_size", 0)
            # device for network inferencing
            inference_device = inf_config.get("inference_device", "cpu")
            net_state_path = pathlib.Path(inf_config.get(
                "network_weights_path"))  # path to the network weights
        ...
        try:
            ...
            cls.classifier_session = onnxruntime.InferenceSession(
                path_or_bytes=net_path)
    ...
    def predict(self, input_img: numpy.ndarray):
        ...
        device_faces = [
            ...
            ).unsqueeze(0).to(self._inference_device)
