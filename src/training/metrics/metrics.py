...
class F1Score(nn.Module):
    ...
    def forward(self,
        ...
        fp = len(numpy.where((predicted_values == 1)
                 & (1 != actual_labels))[0])
        fn = len(numpy.where((predicted_values == 0)
                 & (0 != actual_labels))[0])
        precision = tp / (tp + fn)
        recall = tp / (tp + fp)
...
class IOUScore(nn.Module):
    ...
    def forward(self, pred_boxes: torch.Tensor, actual_boxes: torch.Tensor):
        ...
        pred_area = (pred_boxes[:, 3] - pred_boxes[:, 0]) * \
            (pred_boxes[:, 2] - pred_boxes[:, 1])
        act_area = (actual_boxes[:, 3] - actual_boxes[:, 0]) * \
            (actual_boxes[:, 2] - actual_boxes[:, 1])
...
class AveragePrecision(nn.Module):
    ...
    def forward(self,
        ...
        total_boxes = torch.sum(
            [len(actual_boxes[sample]['boxes']) for sample in actual_boxes])
        ...
        for img_id in pred_boxes:
            ...
            for box1 in range(len(pred_img_boxes)):
                iou_scores = [
                    self.iou(pred_img_boxes[box1],
                             actual_img_boxes[actual_box])
