...
def main():
    ...
    arg("--video-data-dir", type=str, required=True, dest='video_data_dir',
        help='directory, containing video files for processing, available formats: (mp4, mp3)')
    arg("--output-img-dir", type=str, required=True, dest="output_img_dir",
        help='directory to store cropped video frames')
    arg("--total-frames", type=int, required=True, dest='total_frames',
        help='total number of frames to extract from the available dataset videos. (1 frame per video), hence N frames per N videos.')
    ...
    if len(video_paths) == 0:
        raise SystemExit(
            "Failed to find any videos under the provided folder '%s', check the correctness of the path" % video_data_dir)

    print("total number of videos found: %s" % len(video_paths))

    # extracting random video frames from the dataset
    random_indices = numpy.random.randint(
        low=0, high=len(video_paths)-1, size=total_frames)
